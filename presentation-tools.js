...
refreshPresentationFromPlan=function(plan,options={}){
    if(acpMirror && options.notify) {acpPendingPlan=plan;return;}
    if(acpRegie&&options.notify){acpPendingPlan=plan;document.getElementById('acp-pending').hidden=false;return;}
    return acpRefreshOriginal(plan,options);
};
